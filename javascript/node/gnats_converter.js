 ...
 *
 * Parameter MUST be passed as a Buffer not a string.
...
module.exports = exports = (raw) => {
  ...
  for ( let i = 0 ; i < len ; i++ ) {
    ...
    if ( ! (n < 128) ) {
      if ( i > k ) {
        buffer = Buffer.concat([buffer,raw.slice(k,i)])
      }
      if ( (n & 0xe0) == 0xc0 && (n + 1 < len) && (raw[i+1] & 0xc0) == 0x80 ) {
        buffer = Buffer.concat([buffer,raw.slice(i,i+2)])
        i += 1
        k = i + 1
      }
      else if ( (n & 0xf0) == 0xe0 && (n + 2 < len) && (raw[i+1] & 0xc0) == 0x80 && (raw[i+2] & 0xc0) == 0x80 ) {
        buffer = Buffer.concat([buffer,raw.slice(i,i+3)])
        i += 2
        k = i + 1
      }
      else if ( (n & 0xf8) == 0xf0 && (n + 3 < len) && (raw[i+1] & 0xc0) == 0x80 && (raw[i+2] & 0xc0) == 0x80 && (raw[i+3] & 0xc0) == 0x80 ) {
        buffer = Buffer.concat([buffer,raw.slice(i,i+4)])
        i += 3
        k = i + 1
      }
      else {
        buffer = Buffer.concat([buffer,Buffer.from(ascii_extended[n-128])])
        k = i + 1
      }
    }
  }
  if ( k < len )
    buffer = Buffer.concat([buffer,raw.slice(k)])
